...
* **The Problem:** iOS Safari has aggressive, non-standard behaviors when handling the Web Speech API and Gboard/iOS keyboard dictation interactions inside WebGL canvas overlays. Sometimes it swallows the first word, or causes the viewport to rubber-band.
...
* **The Problem:** Selecting text on mobile by dragging a finger across the terminal canvas is currently functional, but double-tap to select a word, or triple-tap to select a line (common mobile paradigms) is missing from the WebGL touch event handlers.
