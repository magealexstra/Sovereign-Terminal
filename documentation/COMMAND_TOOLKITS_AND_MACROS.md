...
* **`/model`**: Switches active LLM model (`/model `).
* **`/plan`**: Initiates planning mode (`/plan `).
...
* **`/learn`**: Saves new behavioral rules or skills (`/learn `).
...
* **`/bug` / `/review`**: Submits bug reports or code reviews.
...
* **Navigation & Function Keys (`NAV`)**:
  * Arrow Keys (`▲`, `▼`, `◀`, `▶`), `PgUp`, `PgDn`, `Home`, `End`, `Ctrl+Left`, `Ctrl+Right`, `Shift+Tab`, `Backspace`, `Enter`, `F1`–`F12`
...
1. **Creating Custom Buttons (Settings -> Sub-Tab 2: Studio)**:
   * Define a **Label**, **Command String** (add `\n` to auto-execute), **Width**, **Height**, **Shape** (Square, Round, Pill), and **Colors**.
   * Custom buttons are saved locally to `sovereign_custom_buttons` and backed up to your server profile.
2. **Building TouchBar Layouts (Settings -> Sub-Tab 3: Layout)**:
   ...
   * Layouts sync automatically across your devices.
